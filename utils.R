...
track2df <- function(ncfile){
  ...
  # Update for new format on 2025-01-24
  ...
  # Get global data
  id <- nc$dim$trajectory$vals  # virtual particle id
  tp <- length(id)  # number of virtual particles
  ts <- nc$dim$obs$len  # number of time steps
  ...
  dt$time <- as.POSIXct(dt$time, origin = "2021-05-01", tz = "UTC")  # seconds since 1968-05-23 00:00:00 GMT
