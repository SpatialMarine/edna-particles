...
for (i in 1:length(bfiles)){
  ...
  sel_times <- seq.POSIXt(from = last(df$time), to = first(df$time), by = "30 min")
  df <- df[df$time %in% sel_times,]
  
  # calculate time from survey
  df$timedif <- difftime(df$time[1], df$time, units = "mins")
  ...
  # map density
  # make a raster map counting the number of particles
  # rc <- rcount(lon = df$lon, lat = df$lat, res = c(0.01, 0.01), crs = CRS("+proj=longlat +datum=WGS84"))
}